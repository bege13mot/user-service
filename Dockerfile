...
WORKDIR $GOPATH/src/github.com/bege13mot/user-service
...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-X main.Version=$(git rev-parse HEAD) -X main.Branch=$(git rev-parse --abbrev-ref HEAD) -X main.BuildTime=$(date -u '+%Y-%m-%d_%H:%M:%S')" -a -installsuffix nocgo -o /user-service .
